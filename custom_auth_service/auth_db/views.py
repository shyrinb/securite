...
def contact_detail(request, contact_id):
    if 'access_token' in request.session:
        ...
        user = get_object_or_404(User, id=user_id) #   admin ou pas
...
def desinscrire_confirm(request, contact_id):
    if 'access_token' in request.session:
        ...
        try:
            contact_to_delete = get_object_or_404(Contact, id=contact_id)
            user_to_delete=User.objects.using('auth_db').filter(contact_relation=contact_to_delete.id).first()
        ...
        return render(request, 'desinscrire.html', {'user_to_delete': user_to_delete,'contact_to_delete': contact_to_delete,'contact_id': contact_to_delete.id})
...
def desinscrire(request, contact_id):
        if 'access_token' in request.session:
            ...
            if request.method == 'POST':
                user_id = request.session.get('user_id')
                contact_to_delete = Contact.objects.using("annuaire_db").get(id=contact_id)
                with transaction.atomic(using='annuaire_db'):
                        # Supprimer l'utilisateur lié manuellement
                        user_to_delete = User.objects.using('auth_db').filter(contact_relation=contact_id).first()
                        if user_to_delete:
                            user_to_delete.delete()

                        # Ensuite, supprimer le contact
                        contact_to_delete.delete()

                        # Enregistrez l'historique de la suppression dans 'auth_db'
                        historique = Historique.objects.using('auth_db').create(
                            user=user_id,
                            action="suppression",
                            commentaire=f"compte de : {contact_id}",
                            timestamp=timezone.now()
                        )
